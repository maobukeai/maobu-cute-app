...
export const AITab: React.FC<AITabProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden cat-bg-canvas transition-colors">
      ...
      <div className="p-2 sm:p-2.5 bg-white/80 dark:bg-[#15151C]/80 backdrop-blur-2xl border-b border-zinc-200/60 dark:border-white/5 shrink-0">
        <div className="flex items-center bg-zinc-100/90 dark:bg-[#1F1F27] p-1 rounded-2xl text-xs select-none border border-zinc-200/40 dark:border-white/5 shadow-inner">
          <button
            onClick={() => {
              sound.playTap();
              setSubTab('chat');
            }}
            className={`flex-1 py-1.5 px-0.5 rounded-xl font-medium transition-all flex items-center justify-center space-x-1 tactile-press ${
              subTab === 'chat'
                ? 'bg-white dark:bg-[#2A2A36] text-zinc-900 dark:text-white shadow-ios-sm font-bold'
                : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-800'
            }`}
          >
            <Bot className="w-3.5 h-3.5 text-[#07C160] shrink-0" />
            <span className="text-[11px] sm:text-xs tracking-tight">智能对话</span>
          </button>
          ...
          >
            <ImageIcon className="w-3.5 h-3.5 text-[#FF6B8B] shrink-0" />
            <span className="text-[11px] sm:text-xs tracking-tight">AI 生图</span>
          </button>
          ...
          >
            <Puzzle className="w-3.5 h-3.5 text-blue-500 shrink-0" />
            <span className="text-[11px] sm:text-xs tracking-tight">技能插件</span>
          </button>
          ...
          >
            ...
            <span className="text-[11px] sm:text-xs tracking-tight">模型配置</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {subTab === 'chat' && (
          <div className="flex-1 flex flex-col h-full overflow-hidden">
            ...
            <div className="px-3 py-1.5 bg-white/70 dark:bg-[#181822]/70 backdrop-blur-md border-b border-zinc-200/50 dark:border-white/5 flex items-center justify-between shrink-0 text-xs gap-1.5">
              {/* Skill quick selector */}
              <div className="flex items-center space-x-1 min-w-0 flex-1">
                <span className="text-sm shrink-0">{activeSkill?.icon || '🐱'}</span>
                <div className="relative min-w-0 flex-1 max-w-[135px] sm:max-w-[200px]">
                  <select
                    value={currentSession?.activeSkillId || skills[0]?.id}
                    onChange={e => handleSelectSkillForSession(e.target.value)}
                    className="w-full bg-zinc-100/90 dark:bg-[#22222E] py-1 pl-2 pr-5 text-[11px] sm:text-xs font-bold text-zinc-800 dark:text-zinc-200 rounded-xl border border-zinc-200/60 dark:border-white/5 outline-none cursor-pointer truncate appearance-none"
                  >
                    {skills.map(s => (
              ...
              <div className="flex items-center space-x-1.5 shrink-0">
                <button
                  ...
                  className="px-2 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/60 text-[10px] sm:text-[11px] text-emerald-700 dark:text-emerald-300 font-mono font-medium hover:opacity-80 flex items-center space-x-1 transition shadow-xs max-w-[105px] sm:max-w-[140px]"
                  title="点击进入模型配置切换端点或模型"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-[#07C160] animate-pulse shrink-0"></span>
                  <span className="truncate">{activeProvider?.defaultModel || '模型配置'}</span>
                  <ChevronDown className="w-2.5 h-2.5 opacity-60 shrink-0" />
                </button>

                <button
                  onClick={handleNewSession}
                  className="w-7 h-7 rounded-xl bg-zinc-100/90 dark:bg-zinc-800/90 text-zinc-700 dark:text-zinc-200 hover:bg-zinc-200 dark:hover:bg-zinc-700 flex items-center justify-center shrink-0 shadow-xs tactile-press"
                  title="新建对话"
                >
            ...
            <div className="p-2.5 sm:p-3 bg-white/90 dark:bg-[#15151C]/90 backdrop-blur-2xl border-t border-zinc-200/60 dark:border-white/5 shrink-0 shadow-ios-sm">
              {/* Quick Prompts with right padding for smooth scroll */}
              <div className="relative mb-2">
                <div className="flex items-center space-x-1.5 overflow-x-auto no-scrollbar py-0.5 px-0.5 pr-6 text-xs">
                  {['今日备忘规划', '写一段温暖问候', 'Python 代码优化', '小红书种草文案'].map(p => (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-full bg-white/90 dark:bg-[#1F1F27] text-zinc-600 dark:text-zinc-300 whitespace-nowrap text-[10.5px] hover:bg-zinc-100 border border-zinc-200/50 dark:border-white/5 shadow-ios-sm tactile-press font-medium shrink-0"
                    >
                      {p}
                    </button>
                  ))}
              ...
              <div className="flex items-end space-x-2">
                <textarea
                  ...
                  className="flex-1 min-h-[40px] max-h-[120px] py-2.5 px-3.5 text-xs sm:text-sm rounded-2xl bg-zinc-100/90 dark:bg-[#1F1F27] border border-zinc-200/50 dark:border-white/5 outline-none resize-none text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 leading-5 focus:ring-2 focus:ring-[var(--theme-accent,#07C160)]/30 transition-all shadow-inner no-scrollbar"
                />

                {isStreaming ? (
                  <button
                    onClick={handleStopStreaming}
                    className="w-10 h-10 rounded-2xl bg-red-500 hover:bg-red-600 text-white shadow-sm transition tactile-press flex items-center justify-center shrink-0"
                    title="停止生成"
                  >
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-2xl transition tactile-press flex items-center justify-center shrink-0 ${
                      inputMessage.trim()
                        ? 'bg-[var(--theme-accent,#07C160)] hover:opacity-95 text-white shadow-sm'
        ...
        {/* ========================================================= */}
        {subTab === 'images' && (
          <div className="flex-1 overflow-y-auto p-3.5 sm:p-4 space-y-4 pb-28">
            {/* Prompt Generator Box */}
            <div className="glass-card p-3.5 sm:p-4 rounded-3xl shadow-ios border border-white/80 dark:border-zinc-800/80 space-y-3">
              ...
              <textarea
                ...
                className="w-full p-3 text-xs rounded-2xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200/50 dark:border-white/5 outline-none text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 resize-none leading-relaxed"
              />

              {/* Style & Size Controls */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs pt-0.5">
                <div className="grid grid-cols-2 gap-2 w-full sm:w-auto">
                  <select
                    value={imageSize}
                    onChange={e => setImageSize(e.target.value)}
                    className="w-full px-2.5 py-2 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border border-zinc-200/50 dark:border-white/5 outline-none font-medium text-xs cursor-pointer"
                  >
                ...
                <button
                  ...
                  className={`w-full sm:w-auto px-5 py-2.5 rounded-2xl text-xs font-bold text-white shadow-sm flex items-center justify-center space-x-1.5 transition tactile-press ${
          ...
          <div className="flex-1 overflow-y-auto p-3.5 sm:p-4 space-y-3 pb-28">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0 flex-1">
                <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 flex items-center space-x-1.5 flex-wrap gap-1">
                  <span>技能插件市场 ({skills.length})</span>
                  <span className="text-[10px] px-1.5 py-0.2 rounded-md bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 font-bold flex items-center space-x-0.5 shrink-0">
